...
| F-03 | 🔴 High | SQL Injection in registration | Registration form accepts raw SQL payloads (e.g., `' ; DROP TABLE users;--`) without validation. Backend processes the request and returns 302 redirect instead of rejecting it. Indicates backend is vulnerable to SQL injection. | Burp Repeater screenshot showing SQLi payload and 302 success response | Not fixed: frontend checks email field but not password field. this means backend doesn't check values that is input in fields. |
